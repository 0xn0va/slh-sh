...
def update_sheet_cell(ws, id_col_values, id_col_value, updating_col_index_header, dbRes):
    ws.update_cell(
...
@app.command()
def update(
    col: Annotated[str, typer.Option(
        help="Column name in Google Sheet")] = "",
    ...
    cov: Annotated[str, typer.Option(
        help="Covidence number as ID")] = "",
    sheet: Annotated[str, typer.Option(
        help="Name of the Sheet")] = configData["gs_studies_sheet_name"],
    gs: Annotated[str, typer.Option(
        help="Google Sheet URL")] = configData["gs_url"],
    idcol: Annotated[str, typer.Option(
        help="Column name that considered as ID e.g 'Covidence #'")] = configData["gs_studies_id_column_name"],
    apply: Annotated[bool, typer.Option(
        help="Apply changes to Google Sheet")] = False,
    allcol: Annotated[bool, typer.Option(
        help="Update all data in column")] = False,
    alltable: Annotated[str, typer.Option(
        help="Add all data of a Table to a new Worksheet")] = "",
):
    '''
    ...
    if cov != "" and col != "" and "," not in cov and "-" not in cov:
        ...
        curr.execute(
            f"SELECT {col} FROM studies WHERE Covidence = '{cov}'")
        ...
        if apply:
            print(
                f"Updating all data in column {col} of {sheet} of Google Sheet...")
        else:
            print(
        ...
        if apply:
            print(
                f"Appending all data in the {alltable} to a new Worksheet...")
            print(gs)
...
def config():
    ...
    if dbRes == None:
        ...
        # create Themes table
        curr.execute('''CREATE TABLE themes
    ...
    if dbRes == None:
        ...
        curr.execute('''CREATE TABLE searches
             (id INTEGER PRIMARY KEY AUTOINCREMENT,
             name TEXT NOT NULL,
    ...
    if dbRes == None:
        ...
        curr.execute('''CREATE TABLE sources
             (id INTEGER PRIMARY KEY AUTOINCREMENT,
             name TEXT NOT NULL,
             description TEXT NOT NULL);''')
        conn.commit()
    ...
    if dbRes == None:
        ...
        curr.execute('''CREATE TABLE annotations
             (id INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    if dbRes == None:
        ...
        curr.execute('''CREATE TABLE distribution
             (id INTEGER PRIMARY KEY AUTOINCREMENT,
             studiesID INTEGER NOT NULL,
             term TEXT NOT NULL,
    ...
    for theme in configData["themes"]:
        ...
        curr.execute(
            f"SELECT id FROM themes WHERE color = '{theme}'")
        dbRes: str = curr.fetchone()
        if dbRes == None:
    ...
    for search in configData["searches"]:
        ...
        curr.execute(
            f"SELECT id FROM searches WHERE name = '{search}'")
        dbRes: str = curr.fetchone()
        if dbRes == None:
    ...
    for source in configData["sources"]:
        ...
        curr.execute(
            f"SELECT id FROM sources WHERE name = '{source}'")
        dbRes: str = curr.fetchone()
        if dbRes == None:
